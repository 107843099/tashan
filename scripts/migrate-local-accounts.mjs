...
const sourceKeys = ['localId', 'username', 'displayName', 'role', 'status', 'updatedAt'];
...
function normalizeLocalRows(rows) {
  ...
  return rows.map(row => {
    ...
    try { username = validateUsername(row.username); displayName = validateDisplayName(row.display_name ?? row.displayName); } catch { fail('LOCAL_DIRECTORY', '本地账号信息不符合当前规则，请先在管理员工作台调整。'); }
    ...
    return { localId, username, displayName, role: row.role, status: row.status, updatedAt: row.updated_at ?? row.updatedAt ?? null };
...
export async function readLocalAccounts(root = ROOT) {
  ...
  try {
    ...
    return normalizeLocalRows(database.prepare('SELECT id, username, display_name, role, status, updated_at FROM accounts ORDER BY id').all());
  } catch { fail('LOCAL_DATABASE', '无法只读访问本地账号目录，请核对数据库与 Node 版本。'); }
...
export function buildMigrationPlan(localRows, cloudRows, config = null) {
  ...
  return {
    ...
    note: '仅迁移账号资料，为新账号分配新密码，登录后即可使用，也可在个人账号页主动修改。不会复制本地密码、修改已有云账号，或合并浏览器项目空间。仅允许将 action 改为 skip；其他内容改变后请重新生成计划。',
...
export async function applyMigration({ plan, localRows, cloudRows, config, administrator, provider, root = ROOT }) {
  ...
  for (const entry of pending) {
    ...
    const item = { localId: entry.localId, username: entry.username, displayName: entry.displayName, role: entry.role, initialPassword: password, state: 'pending', cloudId: null };
    ...
    try {
      const user = await provider.createUser(actor, { username: entry.username, displayName: entry.displayName, role: entry.role, password });
      if (!UUID.test(user?.id || '') || user.username !== entry.username || user.role !== entry.role || user.status !== 'active' || user.mustChangePassword !== false) fail('CREATE_UNCONFIRMED', '云端新增结果尚未确认，请核对账号策略迁移。');
...
async function selfTest() {
  ...
  try {
    const local = [
      { id: randomUUID(), username: 'teacher-one', display_name: '教师一', role: 'member', status: 'active', updated_at: '2026-09-10' },
      { id: randomUUID(), username: 'teacher-two', display_name: '教师二', role: 'admin', status: 'active', updated_at: '2026-09-10' },
    ...
    const provider = { async createUser(actor, input) {
      ...
      assert(files.some(name => JSON.parse(readFileSync(resolve(testRoot, '.local', name))).accounts.some(row => row.username === input.username && row.initialPassword === input.password && row.state === 'pending')));
    ...
    console.log('account migration self-test passed: dry-run, target/source validation, conflict/admin/disabled protection, private credentials before create, explicit new IDs and uncertain-result stop.');
