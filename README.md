...
| 上传与分享 | 两步完成：上传、AI 分析和可编辑教学信息集中在同页，再确认真实预览、测试与使用授权；可直接公开、私有云存储或留在本机 |
...
3. **整理自己的作品。** 在工作台「分享项目」的第一页上传文件或填写 Prompt，同页查看 AI 建议、编辑教学信息，并补充真实效果预览和测试情况。
4. **选择保存或分享。** 第二页确认授权和责任，选择「直接公开发布」或「先存到云端，仅自己可见」，也可「仅保存到此浏览器」。公开后其他成员和游客即可访问；后续保存私有新版不会替换已公开的旧版本。工作台也可管理云端版本和复制公开链接。

两类作品分别校验，避免把说明文字或装饰图片当成项目与效果证明：

| 项目类型 | 公开前需要准备 |
| --- | --- |
| 教学可视化 | 项目文件或完整项目链接；真实截图、动图或视频链接；名称、用途、学科、建议学段、操作者与学习对象、前置知识、预期结果、基本使用方式；运行和实践状态，课堂使用时附记录 |
| Prompt | 名称、用途、完整正文、实际输出截图或动图、实际测试过的 AI 工具；多步骤组合需说明使用顺序和输出依赖 |
| 作者确认 | 明确使用授权，确认真实预览、版权、隐私及内容；Prompt 另需确认教师人工核查 AI 输出 |

音乐、美术、体育与健康等学科已可选择；建议年级和课程关联选填，Prompt 的学科、学段及教学建议也可按需补充。AI 不代替作者确认实际运行、效果证据或授权。服务端在发布时重新检查本人已上传的固定版本，绕过页面无法公开不完整资料；旧公开内容仍可阅读，再次发布时需补全新要求。
...
上传分析默认开启，也可在选文件前关闭。当前可以提取 HTML、Markdown、TXT 和 JSON 的教学文字，自动补充空字段；同页展开「查看 AI 建议，按需替换」，勾选后采用，才会替换已有内容。真实预览、测试工具、运行与课堂记录、许可和责任确认由本人提供，AI 不会自动保存或公开项目。
...
上传流程现已合并为两步，并按可视化与 Prompt 分别校验必备资料；此次功能与测试范围见 [上传流程验收](docs/UPLOAD_WORKFLOW_QA.md)。
...
| 两步上传与发布要求 | [上传流程验收](docs/UPLOAD_WORKFLOW_QA.md) |
