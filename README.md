...
| 账号与个人资料 | 管理员创建、编辑、停用及重置账号；支持学校、机构或个人归属；右上角个人入口先到工作台，再通过「个人资料」按需展开修改密码 |
...
4. **选择保存或分享。** 最后一步可选「仅保存到此浏览器」或「上传并公开」。后者在确认后保存云端版本并发布，其他成员和游客即可访问。工作台也可上传并公开本地项目、发布已有云端版本和复制公开链接。后续修改不会自动替换已公开版本。

工作台的未完成草稿支持继续填写和删除；删除草稿需要确认，只清除未完成内容，不影响已保存项目和云端版本。上传或发布失败时，本地副本会保留以便重试。
...
当前版本已验证账号与权限、上传和固定版本发布、撤回、备份、AI 自动分析，以及三语、明暗和移动端布局。正式域名最近一轮公开验收 **124/124 通过**，实际 DeepSeek 调用和云端文件完整性另行验证。

本次补齐「上传并公开」、工作台发布入口、公开链接复制和草稿删除，详见 [分享与草稿修复验收](docs/SHARING_DRAFT_FIX_QA.md)。
