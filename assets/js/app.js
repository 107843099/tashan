...
function route(){let hash='';try{hash=decodeURIComponent(location.hash.slice(1));}catch{}const [view,id]=hash.split('/');return {view:['discover','project','adapt','guide','desk','upload'].includes(view)?view:'discover',id};}
...
function footer(){return `<footer class="site-footer"><div class="wrap footer-inner"><p>${txt('他山')} · ${txt('让好的教学想法继续生长。')} <button class="subtle brand-replay" type="button" data-action="replay-intro"><span aria-hidden="true">↻</span>${txt('重播开场')}</button></p><p>${txt('本地内容库')} · ${projects.length} ${txt('个项目')} <a href="#guide">${txt('来源与验证说明')} ↗</a></p></div></footer>`;}
...
function workspace(){
 ...
 <aside class="desk-sidebar"><h3>${txt('把作品留在手边')}</h3><p>${txt('上传内容、附件和封面会保存在当前浏览器。导出备份，便于保留或迁移到另一台设备。')}</p><button data-action="export-backup" ${state.busy?'disabled':''}>${txt('导出本地备份')} ↓</button><label class="btn file-label">${txt('导入备份')} ↑<input type="file" id="import-backup" accept="application/json,.json"></label><small>${txt('备份包含上传的项目与草稿；收藏和创作任务保留在当前浏览器。')}</small><a href="#guide">${txt('查看使用指南')} ↗</a>${Object.keys(read('practice-library-draft',{})).length?`<button class="subtle" data-action="restore-legacy-draft">${txt('恢复旧版文字草稿')}</button>`:''}</aside></div>`;
}
...
['工作台数据保存在哪里？','语言选择、收藏、创作任务和文字草稿保存在此浏览器。上传文件、预览图与草稿通过 IndexedDB 保存在此浏览器，可在工作台导出或导入备份。当前未连接账号、AI 或公开发布服务。']
].map(([q,a],i)=>`<details ${i===0?'open':''}><summary>${txt(q)}</summary><p>${txt(a)}</p></details>`).join('')}</section>`;}
function render(){const {view,id}=route();state.renderedView=view;const content=view==='discover'?discover():view==='project'?detail(project(id)):view==='adapt'?adapt(project(id)):view==='desk'?workspace():view==='upload'?upload():guide();document.documentElement.lang=locale;document.title=t('他山 · 教学创作与分享');root.innerHTML=header()+`${state.storageError?`<div class="storage-alert wrap" role="alert">${txt(state.storageError)}</div>`:''}<main class="wrap main-content" id="main" tabindex="-1">${content}</main>`+footer();window.PracticeShowcase?.mount(root);}
...
function taskReference(p){const r={};for(const key of ['id','kind','title','summary','prior','subject','stages','teaching','source','contributors','verification'])r[key]=p[key];return {...r,related:[],duplicates:[],cover:p.isLocal?null:p.cover,sourceHref:p.isLocal?null:p.sourceHref};}
function normalizeDraft(input){const d=input&&typeof input==='object'?{...input}:{};for(const key of ['title','purpose','audience','prior','outcome','setting','stage','subject','core','source','tested','record','kind','used','license'])if(key in d)d[key]=typeof d[key]==='string'?d[key]:'';return d;}
...
function plain(value,fallback=''){return typeof value==='string'?value:fallback;}
...
async function prepareNewDraft(next={}){
 ...
 state.draft=next;state.step=1;state.error='';refreshDraftPreview();await store.putDraft(next);
 if(route().view==='upload')render();else location.hash='upload';
}
async function editProject(id){
 const record=localRecords.find(r=>r.id===id);if(!record)return;
 if(Object.keys(state.draft).length&&state.draft.id!==id){const accepted=await confirmAction('编辑另一个项目？','当前草稿会被替换；如需保留，请先完成保存或导出备份。','继续编辑');if(!accepted)return;}
 state.draft=normalizeDraft(record);state.step=1;refreshDraftPreview();await store.putDraft(state.draft);location.hash='upload';
}
...
async function loadExample(){
 ...
 Object.assign(state.draft,{kind:'prompt',title:t('我的古诗课堂海报'),core:p.document.blocks[0].text,purpose:t('用诗句与画面建立联系，引导学生说出自己的理解。'),subject:'语文',stage:'小学',audience:local(p.teaching?.audience||p.prior),prior:local(p.teaching?.prior||p.prior),outcome:local(p.teaching?.objective||p.summary),setting:local(p.teaching?.method||'教师引导阅读与观察'),source:p.source,tested:t('本次示例由平台制作，尚未课堂试教'),used:'no',coverFile:{name:'poetry-example.png',type:blob.type,blob},sample:true});
...
root.addEventListener('click',async event=>{
 ...
 case 'load-example':b.disabled=true;state.busy=true;await loadExample();state.busy=false;render();break;
 case 'remove-attachment':delete state.draft.attachment;refreshDraftPreview();render();break;
 case 'remove-cover':delete state.draft.coverFile;refreshDraftPreview();render();break;
 case 'restore-legacy-draft':state.busy=true;await prepareNewDraft(normalizeDraft(read('practice-library-draft',{})));state.busy=false;render();break;
 case 'new-project':state.busy=true;await prepareNewDraft();state.busy=false;render();break;
 case 'edit-project':state.busy=true;await editProject(id);state.busy=false;render();break;
 case 'delete-project':if(await confirmAction('删除这个本地项目？','只删除这个上传条目及其附件，原始教学作品库保持可用。','删除项目')){await store.remove(id);localRecords=await store.list();state.saved.delete(id);write('practice-library-bookmarks',[...state.saved]);if(state.draft.id===id){await store.clearDraft();state.draft={};}rebuildProjects();render();toast('本地项目已删除。');}break;
 case 'delete-task':if(await confirmAction('删除这份创作任务？','这会移除工作台保存的任务说明。','删除任务')){delete state.tasks[id];delete state.adapt[id];delete state.briefs[id];write('practice-library-tasks',state.tasks);render();}break;
 case 'export-backup':b.disabled=true;state.busy=true;if(Object.keys(state.draft).length&&!await saveDraft(true))throw new Error(t('草稿未能保存，请点击保存草稿重试。'));showBackup(await store.exportAll());state.busy=false;b.disabled=false;toast('本地备份已准备好');break;
 }
 }catch(error){state.busy=false;b.disabled=false;toast(storageFailure(error));}
});
...
root.addEventListener('change',async event=>{
 ...
 if(node.id==='upload-file'){
  ...
  if(file.size>20*1024*1024)throw new Error(t('项目文件不能超过 20 MB。'));
  ...
  if(/\.(md|markdown|txt)$/i.test(file.name)&&file.size<=1000000){state.draft.kind='prompt';state.draft.core=await file.text();}
  if(!state.draft.title)state.draft.title=file.name.replace(/\.[^.]+$/,'');
  state.busy=false;state.error='';refreshDraftPreview();render();
 }
 if(node.id==='upload-image'){
  ...
  state.draft.coverFile={name:file.name,type:file.type,blob:file};state.busy=false;state.error='';refreshDraftPreview();render();
 }
 if(node.id==='import-backup'){
  const file=node.files[0];if(!file)return;state.busy=true;
  if(Object.keys(state.draft).length&&!await saveDraft(true))throw new Error(t('草稿未能保存，请点击保存草稿重试。'));
  const result=await store.importBackup(file);localRecords=await store.list();state.draft=normalizeDraft(await store.getDraft()||{});state.busy=false;rebuildProjects();render();toast(t('已导入 {count} 个项目',{count:result.count}));
 }
 }catch(error){state.busy=false;state.error=storageFailure(error);if(route().view==='upload'){render();root.querySelector('.error')?.focus();}else toast(state.error);}
});
root.addEventListener('submit',async event=>{
 ...
 if(state.step===1){
  ...
  if(!d.content)state.error='请先确认已检查预览与教学信息。';
   ...
   try{
    const record=await store.put({...d,kind:d.kind||'visual'},{clearDraft:true});
    
    state.draft={};state.busy=false;state.step=1;localRecords=await store.list();rebuildProjects();state.detailTab='teaching';location.hash='project/'+record.id;toast('项目已保存，可以在工作台继续调整。');return;
   }catch(error){state.busy=false;state.error=storageFailure(error);}
  }
 }
 if(!state.error){state.busy=true;const saved=await saveDraft(true);state.busy=false;if(!saved)state.error='草稿未能保存，请点击保存草稿重试。';}
...
window.addEventListener('hashchange',async()=>{
 if(state.renderedView==='upload'&&Object.keys(state.draft).length&&state.storageReady){state.busy=true;const saved=await saveDraft(true);state.busy=false;if(!saved)toast('草稿未能保存，请点击保存草稿重试。');}
...
(async()=>{
 try{
  ...
  const [records,draft]=await Promise.all([store.list(),store.getDraft()]);localRecords=records;
  if(draft)state.draft=normalizeDraft(draft);
  else if(!read('practice-library-migrated-v2',false)){const previous=read('practice-library-draft',{});if(Object.keys(previous).length){state.draft=normalizeDraft(previous);await store.putDraft(state.draft);}}
  write('practice-library-migrated-v2',true);
  state.storageReady=true;rebuildProjects();render();
 }catch(error){state.storageError=storageFailure(error);render();}
})();
})();
