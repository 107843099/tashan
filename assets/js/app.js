...
try{const theme=localStorage.getItem('practice-theme');if(theme==='light'||theme==='dark')document.documentElement.dataset.theme=theme;}catch{}
...
function coverAttributes(p,sizes){
 const variants=Array.isArray(p.coverVariants)?p.coverVariants.filter(v=>typeof v.src==='string'&&Number.isSafeInteger(v.width)&&v.width>0):[];
 return `src="${e(p.cover)}" decoding="async"${variants.length?` srcset="${variants.map(v=>`${e(v.src)} ${v.width}w`).join(', ')}" sizes="${e(sizes)}"`:''}`;
}
function card(p){
 ...
 <a class="card-visual" href="${projectURL(p.id)}" aria-label="${e(local(p.title))}">${p.cover?`<img ${coverAttributes(p,'(max-width: 640px) calc(100vw - 40px), (max-width: 960px) 46vw, 390px')} width="${p.coverWidth||1280}" height="${p.coverHeight||720}" loading="lazy" alt="${e(local(p.title))} · ${txt(p.example?'AI 生成示例':p.isLocal?'上传的预览图':'实际页面截图')}">`:promptCover(p)}</a>
...
function workspace(){
 ...
 ${state.deskTab==='projects'?`${Object.keys(state.draft).length?`<div class="draft-row"><span class="draft-icon">${icon('book-2')}</span><div><small>${txt('未完成的草稿')}</small><strong>${e(state.draft.title||t('未命名实践'))}</strong><p>${txt(state.draft.coverFile?'文字、附件与预览图已保留':'可以随时补充内容和预览图')}</p></div><div class="draft-actions"><a class="btn" href="#upload">${txt('继续填写')} →</a><button class="subtle" data-action="delete-draft" ${state.busy?'disabled':''}>${txt('删除草稿')}</button></div></div>`:''}
 ${localRecords.length?`<div class="local-project-list">${localRecords.map(r=>project(r.id)).filter(Boolean).map(p=>`<article class="local-project-row"><a href="${projectURL(p.id)}">${p.cover?`<img ${coverAttributes(p,'124px')} alt="" width="124" height="84" loading="lazy">`:''}</a><div><span class="subject-label">${txt(p.subject)} · ${txt(p.kind==='prompt'?'Prompt 工具':'教学可视化')}</span><h3><a href="${projectURL(p.id)}">${e(local(p.title))}</a></h3><p><span class="project-code">${e(p.projectCode||p.id)}</span> · ${p.currentVersionNumber?'v'+p.currentVersionNumber+' · ':''}${txt('更新于')} ${e(formatDate(p.updatedAt))}</p></div><div class="local-row-actions">${lifecycle?.localActionsMarkup(p)||''}<button data-action="edit-project" data-id="${e(p.id)}">${txt('编辑')}</button><button class="subtle" data-action="delete-project" data-id="${e(p.id)}">${txt('删除')}</button></div></article>`).join('')}</div>`:`<div class="empty work-empty"><h3>${txt('让下一次好课，从你的作品开始。')}</h3><p>${txt('上传一个可视化文件，或保存一段好用的 Prompt。')}</p><button class="primary" data-action="new-project">${txt('添加第一个项目')}</button></div>`}`
...
function render(){
 ...
 if(accounts){
  if(!accounts.initialized)view='loading';
 ...
 // Account initialization can notify more than once; keep its existing placeholder.
 if(view==='loading'&&state.renderedView==='loading'&&state.loadingLocale===locale)return;
 if(view==='loading')state.loadingLocale=locale;
 state.renderedView=view;
 const content=view==='loading'?`<section class="session-loading" role="status" aria-busy="true"><span class="session-loading-mark" aria-hidden="true">${brandSymbol}</span><p>${txt('正在确认账户…')}</p><span class="session-loading-line" aria-hidden="true"></span></section>`:['login','account','admin'].includes(view)&&accounts?accounts.render(view):view==='cloud'?lifecycle?.remoteMarkup(id)||'':view==='discover'?discover()+(lifecycle?.discoveryMarkup()||''):view==='project'?detail(project(id)):view==='adapt'?adapt(project(id)):view==='desk'?workspace():view==='upload'?upload():guide();
...
root.addEventListener('click',async event=>{
 ...
 const privateActions=['save-brief','brief-to-draft','save-draft','previous-step','load-example','remove-attachment','remove-cover','restore-legacy-draft','new-project','edit-project','delete-draft','delete-project','delete-task','export-backup','export-guest-backup','reference-add','reference-remove','copy-saved-task','copy-brief'];
 ...
 case 'theme':{setLanguageMenu(false);const dark=document.documentElement.dataset.theme!=='dark';document.documentElement.dataset.theme=dark?'dark':'light';try{localStorage.setItem('practice-theme',dark?'dark':'light');}catch{}const label=t(dark?'切换浅色':'切换深色');b.innerHTML=headerIcons[dark?'sun':'moon'];b.setAttribute('aria-label',label);b.title=label;b.focus({preventScroll:true});break;}
 ...
 }catch(error){if(interactionEpoch!==workspaceEpoch)return;state.busy=false;b.disabled=false;if(b.dataset.action==='delete-draft')render();toast(storageFailure(error));}
});
