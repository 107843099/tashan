      ...
      <div class="guide-layout">
        ...
        <div class="guide-content">
          ...
          ${section('publish', '记住这三个状态，就不会弄混作品保存到了哪里。', `<ol class="guide-states">
            ...
            ${item('公开发布', '在云端详情的“查看已上传版本”中选择版本，点击“发布此版本”，核对后“确认发布”。游客才能查看该版本，你可以复制公开链接分享。')}
          ...
          ${section('account', '自己的密码自己修改，账号由管理员统一维护。', `<div class="guide-account-options"><section><h3>${txt('我是普通成员')}</h3>${p('点击右上角的账户入口，进入“我的账户”。可查看资料、填写当前密码与新密码，并退出登录。新密码至少 8 位，允许纯数字；修改显示名称请联系管理员。')}<a class="guide-text-link" href="#account">${txt('前往我的账户')} <span aria-hidden="true">↗</span></a></section><section><h3>${txt('我是管理员')}</h3>${p('从“我的账户”进入“账户管理”，用“新建账户”填写用户名、显示名称、角色与初始密码。选中列表中的账户后，可修改资料、重置密码或启停用。')}${p('搜索可查询全部账户，角色和状态筛选只作用于当前页。新建账户不要求首次改密；管理员重置密码后，用户需要设置新密码。')}${isAdmin ? `<a class="guide-text-link" href="#admin">${txt('打开账户管理')} <span aria-hidden="true">↗</span></a>` : ''}</section></div>`)}

          ${section('questions', '遇到问题，先检查这里。', [
            ['为什么保存了，别人却看不到？', '本地保存、上传云端和公开发布是三个独立步骤。先检查云端项目是否已有上传版本，再确认该版本已公开发布。只有公开版本可以分享给游客。'],
