...
export function createStoneScene(container,{reduced=false,onLost=()=>{}}={}){
  ...
  const jadeGeo=keep(new THREE.TorusGeometry(.70,.34,48,128));jadeGeo.scale(.94,1.08,.48);
  ...
  const jade=new THREE.Mesh(jadeGeo,jadeMaterial);assembly.add(jade);jade.visible=false;
  ...
  let width=1,height=1,frame=0,disposed=false,paused=reduced,started=performance.now(),motionTime=0,last=started,carveStart=null,frozenTime=null,px=0,py=0,aimX=0,aimY=0,baseX=0,baseY=0;
  const clamp=n=>Math.max(0,Math.min(1,n)),ease=n=>1-Math.pow(1-clamp(n),3);
  function resize(){if(disposed)return;const rect=container.getBoundingClientRect();width=rect.width;height=rect.height;renderer.setSize(width,height);camera.aspect=width/height;camera.updateProjectionMatrix();const visibleHeight=2*Math.tan(THREE.MathUtils.degToRad(camera.fov/2))*camera.position.z,visibleWidth=visibleHeight*camera.aspect;const mobile=width<768;baseX=mobile?0:visibleWidth*.19;baseY=mobile?-visibleHeight*(height<700?.12:.10):-visibleHeight*.015;assembly.scale.setScalar(mobile?Math.min(.61,height/950):Math.min(.94,width/1200));draw(performance.now(),true);}
